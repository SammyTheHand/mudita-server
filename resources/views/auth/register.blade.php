...
@section('content')
    <form method="POST" action="{{ route('register') }}"
          class="lg:w-1/2 lg:mx-auto bg-blue-400 py-12 px-16 rounded shadow"
    >
        @csrf
        ...
        <div class="field mb-6">
            ...
            <div class="control">
                <input id="name"
                       type="text"
                       class="input border border-muted-light rounded p-2 text-xs w-full{{ $errors->has('name') ? ' is-invalid' : '' }}"
                       name="name"
                       value="{{ old('name') }}"
                       required
                       autofocus>
        ...
        <div class="field mb-6">
            ...
            <div class="control">
                <input id="email"
                       type="email"
                       class="input border border-muted-light rounded p-2 text-xs w-full{{ $errors->has('email') ? ' is-invalid' : '' }}"
                       name="email"
                       value="{{ old('email') }}"
                       required>
            </div>
